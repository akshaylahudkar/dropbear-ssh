...
# Library scriptlet install.sh adds — safe to remove unconditionally, it's
# entirely generated by this package, nothing user-authored lives in it.
# The .sdr sidecar (Kindle's per-book metadata dir) only appears once the
# device has actually indexed the file, so it may not exist yet.
